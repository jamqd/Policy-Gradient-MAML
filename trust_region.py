...
def compute_advantages(baseline, tau, discount, rewards, dones, states, next_states):
    # Update baseline
...
def maml_a2c_loss(train_episodes, learner, baseline, discount, tau):
    ...
    advantages = compute_advantages(baseline, tau, discount, rewards,
                                    dones, states, next_states)
    advantages = ch.normalize(advantages).detach()
    return a2c.policy_loss(log_probs, advantages)


def fast_adapt_a2c(clone, train_episodes, adapt_lr, baseline, discount, tau, first_order=False):
    second_order = not first_order
    loss = maml_a2c_loss(train_episodes, clone, baseline, discount, tau)
...
def meta_surrogate_loss(iteration_replays, iteration_policies, policy, baseline, tau, discount, adapt_lr):
    mean_loss = 0.0
    mean_kl = 0.0
    for task_replays, old_policy in tqdm(zip(iteration_replays, iteration_policies),
        ...
        for train_episodes in train_replays:
            new_policy = fast_adapt_a2c(new_policy, train_episodes, adapt_lr,
                                        baseline, discount, tau, first_order=False)
        ...
        advantages = compute_advantages(baseline, tau, discount, rewards, dones, states, next_states)
        advantages = ch.normalize(advantages).detach()
...
def maml_trpo(
    ...
    for iteration in range(num_iterations):
        ...
        for task_config in tqdm(env.sample_tasks(meta_batch_size), leave=False, desc='Data'):  # Samples a new config
            ...
            valid_episodes = task.run(clone, episodes=adapt_batch_size)
        ...
        old_loss, old_kl = meta_surrogate_loss(iteration_replays, iteration_policies, policy, baseline, tau, discount,
                                               adapt_lr)
        ...
        for ls_step in range(ls_max_steps):
            ...
            new_loss, kl = meta_surrogate_loss(iteration_replays, iteration_policies, clone, baseline, tau, discount,
                                               adapt_lr)
